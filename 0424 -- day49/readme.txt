...
1- 服务器自动重启
	...
	使用:
		...
		使用nodemon 
		nodemon 文件名  nodemon app.js
	通过nodemon app.js启动的服务,则会监事文件的变化,当文件发生变化是,自动重启服务器
2- 在express框架中使用模板引擎
	1- art-template - Github仓库	https://aui.github.io/art-template
	2- art-template官方文档	https://aui.github.io/art-template/
	3- 安装
	...
	5- 使用:
		app.get("/",function(req.res){
			//express默认会去项目中找到views目录中找index.html
			res.render("index.html",{
				title:"大华1903最棒"
			})
		})
	6- 如果你想修改默认的views视图渲染存储目录,
		切记:第一个参数views千万别写错
		app.set("views",目录路径)
3- 使用express重写评论功能
4- 基本路由 (post) 在express 获取表单post请求体数据
	...
	配置:
		...
		app.use(function (req, res) {
		  ...
		  //可以通过req.body来获取表单post请求体的数据
		  res.end(JSON.stringify(req.body, null, 2))
		})
		
	在express 获取表单get请求参数
	express内置一个api,可以通过 req.query来进行获取
